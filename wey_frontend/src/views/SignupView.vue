    ...
    <div class="max-w-7xl mx-auto grid grid-cols-2 gap-4">
        <div class="main-left">
            <div class="p-12 bg-white border border-gray-200 rounded-lg">
                ...
                <p class="font-bold">
                    Есть аккаунт? <RouterLink :to="{'name': 'login'}" class="underline">Войти</RouterLink>
                </p>
            </div>
        </div>
        ...
        <div class="main-right">
            <div class="p-12 bg-white border border-gray-200 rounded-lg">
                <form class="space-y-6" v-on:submit.prevent="submitForm">
                    <div>
                        <label>Имя</label><br>
                        <input type="text" v-model="form.name" placeholder="введите свое имя..." class="w-full mt-2 py-4 px-6 border border-gray-200 rounded-lg">
                    </div>
...
export default {
    ...
    mounted() {
        document.title = "Регистрация | Почтальон"
    ...
    methods: {
        submitForm() {
            ...
            if (this.form.password1 !== this.form.password2) {
                this.errors.push('Папроли не совпали')
            }
                ...
                axios
                    ...
                    .then(response => {
                        if (response.data.message === 'success') {
                            this.toastStore.showToast(5000, 'Пользователь зарегистрирован. Пожалуйста, активируйте свою учетную запись, нажав на ссылку электронной почты.', 'bg-emerald-500')
